...
def push(local, repo_path=None):
    ...
    sha = None
    try:
        r = requests.get(url, headers=H, proxies=PROXIES, timeout=30,
                         params={"ref": BRANCH})
        if r.status_code == 200:
            sha = r.json().get("sha")
    except Exception as e:
        print(f"WARN {repo_path} GET sha 失败: {str(e)[:60]}")
    body = {"message": f"update {repo_path} via gh_push", "content": content,
            "branch": BRANCH}
    if sha:
        body["sha"] = sha
    r = requests.put(url, headers=H, proxies=PROXIES, json=body, timeout=60)
